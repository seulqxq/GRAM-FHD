...
class GramRF(nn.Module):
    def __init__(self, z_dim=100, feature_dim=0, hidden_dim=256, sigma_clamp_mode='softplus', rgb_clamp_mode='widen_sigmoid'):
        super().__init__()
        self.z_dim = z_dim
        ...
        if (self.feature_dim > 0): self.output_feature.apply(frequency_init(25))
    ...
    def get_avg_w(self):
        ...
        self.avg_frequencies, self.avg_phase_shifts = self.mapping_network.from_w(self.avg_w)
        return self.avg_frequencies, self.avg_phase_shifts, (z, ws)
    ...
    def forward_feature_with_frequencies_phase_shifts(self, input, frequencies, phase_shifts, ray_directions, eps=1e-3):
        frequencies = frequencies*15 + 30
...
class Gram(nn.Module):
    ...
    def pretrained_params(self):
        parameters = [p for n, p in self.named_parameters() if p.requires_grad and 'output_feature' not in n]
        return parameters
